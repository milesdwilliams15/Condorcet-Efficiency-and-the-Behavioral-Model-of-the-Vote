...
x <- sample(seq(0,1,.01),replace=T,size=25001)
k <- c(.39,.47,.71)
df <- data.frame(voter=rep(1:25001,len=length(x)*3),
                 cand=rep(c('A','B','C'),len=length(x)*3),
                 ideal_voter=rep(x,len=length(x)*3),
                 ideal_cand=rep(k,len=length(x)*3),
                 noise=exp(-exp(-rnorm(length(x)*3,sd=sqrt(0.7),
                                       mean=0.6))) ) %>% 
